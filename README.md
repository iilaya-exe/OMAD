...
- **Over-budget banner** at the top whenever the month's spending passes your overall budget or any category limit
- Spending-by-category bar chart with a consistent color per category, a dashed tick marking each category's limit, and any overspend drawn as a hatched red segment past that tick (labelled "over by ₱X")
...
**Budgets**
- **Overall monthly budget** — one general limit covering everything you spend in a month
- Optional monthly limit per category, with a note showing how much of the overall budget they allocate (and a warning if they add up to more than it)
- Progress meters that turn amber near the limit and red (with a ⚠ warning and the overage amount) when you go over
- The dashboard measures against your overall budget when it's set, and falls back to the total of your category budgets when it isn't
...
- **Accessible by default:** ARIA roles on tabs/dialog/charts, and both light and dark themes use palettes tuned for their own surface.
- **Over-budget never rests on color.** Wherever spending passes a limit it is marked four ways at once: a red segment, a diagonal hatch, a ⚠ glyph, and text naming the overage — so it survives grayscale printing and color-vision differences.
